...
Run once per system (complex or ligand):
    python 04_reconstruct.py --config config_ixazomib_complex_sto-3g.yaml
    python 04_reconstruct.py --config config_bortezomib_complex_sto-3g.yaml
    python 04_reconstruct.py --config config_ixazomib_ligand_sto-3g.yaml
    python 04_reconstruct.py --config config_bortezomib_ligand_sto-3g.yaml
"""
...
# Derive a human-readable system label from the run name
system_label = _run_name.replace("_sto-3g", "").replace("_", " ")
...
# ---------------------------------------------------------------------------
# Build EmbeddingResult from pkl — no Vayesta re-kernel needed
# ---------------------------------------------------------------------------
...
print(f"\n{'=' * 60}")
print(f"EWF+SQD Energy Reconstruction  —  {system_label}")
print(f"{'=' * 60}")
print(f"  HF energy:          {hf_energy:.8f} Ha")
...
for frag_id, result in fragment_results.items():
    ...
    if n_orb >= orbital_threshold and not has_ecorr:
        ...
        solver = "FCI"
    print(f"  {frag_id:>4}  {solver:>5}  {n_orb:>6}  "
